...
namespace BitMexSampleBot
{
    public partial class Form1 : Form
    {
        ...
        private void LoadAPISettings()
        {
            ...
            {
                case "TestNet":
                    textBoxKey.Text = Properties.Settings.Default.TestAPIKey;
                    textBoxSecret.Text = Properties.Settings.Default.TestAPISecret;
                    break;
                case "RealNet":
                    textBoxKey.Text = Properties.Settings.Default.APIKey;
                    textBoxSecret.Text = Properties.Settings.Default.APISecret;
                    break;
            }
        }
        ...
        private void MakeOrder(string Side, int Qty, double Price = 0)
        {
            ...
            switch((string)ddlOrderType.SelectedItem)
            {
        ...
        {
            switch ((string)ddlAutoOrderType.SelectedItem)
            {
                case "Limit Post Only":
                    if (Price == 0)
                    {
        ...
        private void UpdateCandles()
        {
                            ...
                            //  However, if you wanted to use info from the most current candle, that wouldn't work well
            {
                ...
                btnAutomatedTrading.Text = "Press to stop. Mode: " + Mode;// so we can see what the mode of the bot is while running
            }
        }
        ...
        private void SetBotMode()
        {
           ...
           /* if(rdoBuy.Checked)
            ...
            if (rdoBuy.Checked)
            {
                ...
                if ((Candles[0].MA1 > Candles[0].MA2)) // Most recently closed candle crossed over up
                {
                    // Did the last candle have MA1 cross above MA2?  We'll need to buy now.
                    Mode = "Buy";
                }

                else if ((Candles[0].MA1 > Candles[0].MA2) && (Candles[1].MA1 > Candles[1].MA2))
                {
                    // If no crossover, is MA1 still above MA2? We'll need to leave our position open.
                    Mode = "Wait";
                }
                
                else if ((Candles[0].MA1 < Candles[0].MA2))
                {
                    // Did the last full candle have MA1 cross below MA2?  We'll need to close any open position.
                    Mode = "Close";
                }
        ...
        private void btnAutomatedTrading_Click(object sender, EventArgs e)
        {
            if(btnAutomatedTrading.Text == "Start")
            {
                ...
                btnAutomatedTrading.Text = "Press to stop. Mode:  " + Mode;
            ...
            else
            {
                ...
                Running = false;
                rdoBuy.Enabled = true;
                rdoSell.Enabled = false; //kolkas ijungiam 
                rdoSwitch.Enabled = false; // NEW
            }
        ...
        private void textBoxKey_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                case "TestNet":
                    Properties.Settings.Default.TestAPIKey = textBoxKey.Text;
                    break;
                case "RealNet":
                    Properties.Settings.Default.APIKey = textBoxKey.Text;
                    break;
            }
            SaveSettings();
            InitializeAPI();
        ...
        private void textBoxSecret_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                case "TestNet":
                    Properties.Settings.Default.TestAPISecret = textBoxSecret.Text;
                    break;
                case "RealNet":
                    Properties.Settings.Default.APISecret = textBoxSecret.Text;
                    break;
            }
            SaveSettings();
            InitializeAPI();
